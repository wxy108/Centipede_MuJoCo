...
def convert_leg_actuators_to_general(xml):
    """Convert all <position name="act_joint_leg_..." ...> and
    <position name="act_joint_foot_..." ...> to <general gainprm="1"> (torque mode)."""
    count = 0
    def repl(m):
        nonlocal count; count += 1
        name, joint = m.group(1), m.group(2)
    ...
    xml = re.sub(
        r'<position name="(act_joint_(?:leg|foot)_[^"]*)" joint="([^"]*)" '
        r'kp="[^"]*" kv="[^"]*" forcelimited="[^"]*" forcerange="[^"]*"\s*/>',
        repl, xml)
    return xml, count
...
def add_pitch_actuators(xml):
    ...
    # Find all pitch joint names (extract just the name attribute)
...
def main():
    ...
    parser.add_argument("--convert-legs", action="store_true",
                        help="Convert leg <position> actuators to <general> (torque mode)")
    ...
    # 5. Optionally convert leg actuators to torque mode
    n_leg_conv = 0
    if args.convert_legs:
        xml, n_leg_conv = convert_leg_actuators_to_general(xml)
        print(f"[5] Converted {n_leg_conv} leg actuators → general (torque mode)")
    ...
    if n_leg_conv:
        print(f"Leg actuators converted to torque mode: {n_leg_conv}")
